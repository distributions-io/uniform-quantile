...
function quantile( p, options ) {
	...
	if ( opts.b <= opts.a ) {
		throw new TypeError( 'quantile()::invalid input options. Parameter `a` must be smaller than `b`.' );
	}

	if ( isNumber( p ) ) {
		return quantile1( p, opts.a, opts.b );
	}
	if ( isMatrixLike( p ) ) {
		if ( opts.copy !== false ) {
		...
		return quantile5( out, p, opts.a, opts.b );
	}
	if ( isTypedArrayLike( p ) ) {
		...
		return quantile6( out, p, opts.a, opts.b );
	}
	if ( isArrayLike( p ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return quantile4( p, opts.a, opts.b, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return quantile3( out, p, opts.a, opts.b, opts.accessor );
		}
		return quantile2( out, p, opts.a, opts.b );
	}
	return NaN;
